...
const notasData = [
  { materia: "Matemáticas", evaluacion: "Prueba 1", nota: 6.5, porcentaje: 30, fecha: "2026-04-10" },
  { materia: "Matemáticas", evaluacion: "Prueba 2", nota: 5.8, porcentaje: 30, fecha: "2026-05-05" },
  { materia: "Física", evaluacion: "Examen parcial", nota: 7.0, porcentaje: 40, fecha: "2026-04-20" },
  { materia: "Química", evaluacion: "Laboratorio", nota: 6.0, porcentaje: 20, fecha: "2026-05-01" },
  { materia: "Historia", evaluacion: "Ensayo", nota: 5.5, porcentaje: 25, fecha: "2026-05-10" },
  { materia: "Lenguaje", evaluacion: "Prueba oral", nota: 6.8, porcentaje: 35, fecha: "2026-05-15" },
  { materia: "Biología", evaluacion: "Prueba 1", nota: 4.5, porcentaje: 30, fecha: "2026-04-28" },
];

const chartData = [
  { materia: "Mat", promedio: 6.15 },
  { materia: "Fís", promedio: 7.0 },
  { materia: "Quí", promedio: 6.0 },
  { materia: "His", promedio: 5.5 },
  { materia: "Len", promedio: 6.8 },
  { materia: "Bio", promedio: 4.5 },
];
...
export default function NotasPage() {
  ...
  const [notas, setNotas] = useState(notasData.map(n => n.nota.toString()));
  ...
  const promedio = notasData.reduce((acc, n) => acc + n.nota, 0) / notasData.length;

  // Calculadora para aprobar
  const calcularNotaRequerida = () => {
    const min = parseFloat(notaMinima);
    const pend = parseFloat(porcentajePendiente);
    if (isNaN(min) || isNaN(pend) || pend <= 0) return;
    const notasActuales = notasData.reduce((acc, n) => acc + (n.nota * n.porcentaje), 0) /
      notasData.reduce((acc, n) => acc + n.porcentaje, 0);
  ...
  const promedioCalculado = notas.reduce((acc, n) => {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: "Promedio General", value: promedio.toFixed(1), color: promedio >= 6 ? "green" : promedio >= 5 ? "yellow" : "red" },
            { label: "Evaluaciones", value: notasData.length.toString(), color: "blue" },
            { label: "Aprobadas", value: notasData.filter(n => n.nota >= 6).length.toString(), color: "green" },
          ].map((s, i) => (
            <motion.div
              key={s.label}
              initial={{ opacity: 0, y: 20 }}
                ...
                {filtered.map((n, i) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3">
                      <span className={`text-sm font-bold ${getColor(n.nota)}`}>
                        {n.nota.toFixed(1)}
                      </span>
                    </td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3 text-sm text-gray-500">{n.fecha}</td>
                    <td className="px-5 py-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className={`p-4 rounded-xl ${isNaN(promedioCalculado) ? "bg-gray-50" : promedioCalculado >= 6 ? "bg-green-50" : promedioCalculado >= 5 ? "bg-yellow-50" : "bg-red-50"}`}>
              <p className="text-xs text-gray-500">Promedio calculado</p>
              <p className={`text-3xl font-bold ${isNaN(promedioCalculado) ? "text-gray-400" : promedioCalculado >= 6 ? "text-green-600" : promedioCalculado >= 5 ? "text-yellow-600" : "text-red-600"}`}>
                {isNaN(promedioCalculado) ? "—" : promedioCalculado.toFixed(1)}
              </p>
              <p className="text-xs mt-1 text-gray-500">
                {!isNaN(promedioCalculado) && (promedioCalculado >= 6 ? "✅ Aprobado" : promedioCalculado >= 5 ? "⚠️ En riesgo" : "❌ Reprobado")}
              </p>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-gray-800">¿Qué necesito para aprobar?</h3>
            </div>
            <div className="space-y-3 mb-4">
              <div>
                <label className="text-xs text-gray-500 mb-1 block">Nota mínima para aprobar</label>
                <input
                  type="number"
                  min="1"
                  max="7"
            ...
            {notaRequerida !== null && (
              <motion.div
                ...
                className={`p-4 rounded-xl ${notaRequerida <= 7 ? notaRequerida <= 6 ? "bg-green-50" : "bg-yellow-50" : "bg-red-50"}`}
              >
                <p className="text-xs text-gray-500">Necesitas obtener</p>
                <p className={`text-3xl font-bold ${notaRequerida <= 6 ? "text-green-600" : notaRequerida <= 7 ? "text-yellow-600" : "text-red-600"}`}>
                  {notaRequerida > 7 ? "Imposible" : notaRequerida < 1 ? "Ya aprobaste" : notaRequerida.toFixed(1)}
                </p>
                <p className="text-xs mt-1 text-gray-500">
                  {notaRequerida > 7 ? "❌ No es posible aprobar" : notaRequerida < 1 ? "✅ Ya tienes la nota suficiente" : notaRequerida <= 6 ? "✅ Es alcanzable" : "⚠️ Será difícil pero posible"}
                </p>
              </motion.div>
            )}
          </motion.div>
